...
export class RunnerGame {
  ...
  private frame = (now: number) => {
    ...
    // On phones the previous camera sat farther back, making the philosopher look too small.
    // Bring only the mobile camera closer; desktop framing stays unchanged.
    this.camera.position.set(cb * .22, narrow ? 5.8 : 4.9, narrow ? 9.8 : 10.5);
    this.camera.lookAt(cb * .5, 1.15, narrow ? -15.5 : -18);
