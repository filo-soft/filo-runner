...
proto.spawn = function () {
  ...
  if (t < 25) {
    ...
    if (r % 3 === 0) {
      add('block', blocked[0], -80);
      add('pillar', blocked[1], -80);
    } else if (r % 3 === 1) {
      add('arch', blocked[0], -80);
      add('pillar', blocked[1], -80);
    } else {
      add('block', blocked[0], -80);
      add('arch', blocked[1], -80);
    }
    ...
    const pattern = r % 4;
    if (pattern === 0) {
      add('block', lanes[0], -80); add('pillar', lanes[1], -80);
      add('arch', safe, -96);
    } else if (pattern === 1) {
      add('arch', lanes[0], -80); add('block', lanes[1], -80);
      add('pillar', safe, -96);
    } else if (pattern === 2) {
      add('pillar', lanes[0], -80); add('arch', lanes[1], -80);
      add('block', safe, -96);
    } else {
      add('block', lanes[0], -80); add('arch', lanes[1], -80);
      add('pillar', lanes[0], -98);
    }
...
proto.step = function (dt: number) {
  const t = this.stats.time as number;
  const factor = 1 + Math.min(.28, Math.max(0, t - 8) * .0035);
  originalStep.call(this, dt * factor);
};

proto.buildPrototypes = function () {
  originalBuildPrototypes.call(this);
  const arch = this.prototypes.get('arch');
  if (!arch) return;
  arch.traverse((o: any) => {
    if (!o.isMesh) return;
    if (o.position.y <= 1.6 && (o.scale?.y ?? 1) <= 1.5) return;
    const mat = Array.isArray(o.material) ? o.material.map((x: any) => x.clone()) : o.material.clone();
    const makeTransparent = (m: any) => { m.transparent = true; m.opacity = .16; m.depthWrite = false; };
    if (Array.isArray(mat)) mat.forEach(makeTransparent); else makeTransparent(mat);
    o.material = mat;
  });
};
