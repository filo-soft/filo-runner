...
function makeIcon(kind: 'magnet' | 'shield') {
  ...
  if (kind === 'magnet') {
    ...
    const shield = new T.Mesh(new T.ExtrudeGeometry(shape, { depth: .14, bevelEnabled: true, bevelSize: .04, bevelThickness: .04 }), body); shield.position.z = -.07; root.add(shield);
  }
...
function spawnRamp(game: any) {
  ...
  for (let i = game.items.length - 1; i >= 0; i--) {
    ...
    if (item.type !== 'coin' && item.type !== 'gate' && item.mesh.position.z > RAMP_Z - 6 && item.mesh.position.z < RAMP_Z + RAMP_SAFE_END) {
      game.recycle(item); game.items.splice(i, 1);
    }
  }
...
proto.die = function () {
  ...
  if (state.shieldUntil > this.stats.time) {
    this.shake = .05; this.burst(new T.Vector3(this.runner.position.x, this.groundY + .8, 1.2), true, 10); this.tone(240, .08, 520, 'sine'); this.onToast('Щит выдержал удар'); return;
  }
  ORIGINALS.die.call(this);
};

proto.step = function (dt: number) {
  const state = ensureState(this); const beforeTravel = this.travel as number;
  ...
  if ((this.travel as number) >= state.nextRampTravel && !RAMPS.some(r => r.z < 20 && r.z > -130)) {
    spawnRamp(this); state.nextRampTravel = (this.travel as number) + RAMP_INTERVAL_MIN + Math.random() * (RAMP_INTERVAL_MAX - RAMP_INTERVAL_MIN);
  }
  ...
  for (const ramp of RAMPS) {
    ...
    if (rel >= RAMP_START && rel < RAMP_TOP_START) { rampY = Math.max(rampY, (rel / RAMP_TOP_START) * 2); onRamp = true; }
  ...
  if (state.magnetUntil > now) {
    ...
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      if (Math.abs(dz) < .9 && Math.abs(item.mesh.position.x - this.runner.position.x) < 1.0) {
        this.stats.coins++; this.burst(item.mesh.position); this.tone(700 + (this.stats.coins % 5) * 130, .12, 1350); this.recycle(item); this.items.splice(i, 1);
      }
    }
  }
