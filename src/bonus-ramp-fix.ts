...
const ORIGINALS = { start: proto.start, step: proto.step, die: proto.die };
...
function makeBonusVisual(item: any, type: BonusType) {
  ...
  if (type === 'magnet') {
    ...
    const shape = new T.Shape(); shape.moveTo(0, .42); shape.lineTo(.36, .18); shape.lineTo(.28, -.18); shape.lineTo(0, -.42); shape.lineTo(-.28, -.18); shape.lineTo(-.36, .18); shape.closePath();
...
function removeItem(game: any, item: any) {
  ...
  if (index >= 0) { game.recycle(item); game.items.splice(index, 1); }
}
...
function rampSurfaceY(game: any, item: any) {
  ...
  for (const ramp of ramps) {
    ...
    if (ramp.type === 'ramp' && rel >= -1.9 && rel <= 2.9) {
      const p = T.MathUtils.clamp((2.9 - rel) / 4.8, 0, 1); height = p < .82 ? p * 1.65 : 1.65;
    } else if (ramp.type === 'stairRamp' && rel >= -1.9 && rel <= 3.25) {
      const p = T.MathUtils.clamp((3.25 - rel) / 4.5, 0, 1); height = p < .82 ? p * 1.84 : 1.84;
    }
...
function keepBonusAboveRamp(game: any, item: any) {
  ...
  if (y !== null) item.mesh.position.y = Math.max(item.mesh.position.y, y + .28);
  else if (item.mesh.position.y > 1.5) item.mesh.position.y = Math.max(item.mesh.position.y, 2.6);
}
function activateBonus(game: any, item: any) {
  const state = ensureState(game); const now = game.stats.time as number;
...
function updateBonusHud(game: any, state: any) {
  ...
  if (!hud) {
    ...
    const style = document.createElement('style'); style.textContent = `
      ...
      .bonus-dot { width:9px; height:9px; border-radius:50%; } .bonus-dot.magnet { background:#e94b62; } .bonus-dot.shield { background:#4b72e8; }
      @media(max-width:700px) { .bonus-status { top:205px; left:12px; bottom:auto; max-width:calc(100vw - 24px); font-size:9px; gap:6px; } .bonus-badge { padding:6px 8px; } }
...
proto.die = function () {
  ...
  if (state.shieldUntil > this.stats.time) { this.shake = .05; this.burst(new T.Vector3(this.runner.position.x, this.groundY + .8, 1.2), true, 10); this.tone(240, .08, 520, 'sine'); this.onToast('Щит выдержал удар'); return; }
...
proto.step = function (dt: number) {
  ...
  if (state.magnetUntil > now && MAGNET_ENABLED) {
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      const pull = Math.min(1, dt * 12); const targetX = this.runner.position.x;
      item.laneX = T.MathUtils.lerp(item.laneX, targetX - this.bendOff(item.mesh.position.z), pull); item.mesh.position.x = item.laneX + this.bendOff(item.mesh.position.z); item.mesh.position.y = T.MathUtils.lerp(item.mesh.position.y, this.groundY + this.jump + .9, pull);
      if (Math.abs(dz) < .9 && Math.abs(item.mesh.position.x - targetX) < 1.0) { this.stats.coins++; addPersistentCoins(1); this.burst(item.mesh.position); this.tone(700 + (this.stats.coins % 5) * 130, .12, 1350); this.recycle(item); this.items.splice(i, 1); }
    }
  }
  if (state.magnetUntil > now && MAGNET_ENABLED) for (const item of this.items as any[]) if (item.type === 'coin') { const dz = item.mesh.position.z - 1.2; if (dz >= -120 && dz <= 3) { item.laneX = T.MathUtils.lerp(item.laneX, this.runner.position.x - this.bendOff(item.mesh.position.z), Math.min(1, dt * 12)); item.mesh.position.x = item.laneX + this.bendOff(item.mesh.position.z); } }
