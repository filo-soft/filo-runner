...
const cloneBlueGuard = (game: IvanGame) => {
  ...
  for (const mesh of guardMeshes) {
    ...
    const next = mats.map((mat: any) => {
      ...
      const hsl = { h: 0, s: 0, l: 0 };
      color.getHSL(hsl);
      if (color.getHex() === 0xe2ddcf || (hsl.s < .12 && hsl.l > .78 && hsl.l < .95)) return blue.clone();
...
const showIntroIvan = (game: IvanGame) => {
  cloneBlueGuard(game);
  if (!game.__ivanGuardRoot) return;
  game.__ivanIntroRemaining = IVAN_INTRO_SECONDS;
  game.__ivanGuardGap = IVAN_START_GAP;
...
const updateIvan = (game: IvanGame, dt: number) => {
  ...
  if ((game.__ivanIntroRemaining ?? 0) > 0) {
    game.__ivanIntroRemaining = Math.max(0, (game.__ivanIntroRemaining ?? 0) - dt);
  }
  ...
  if (state === 'retreat') {
    ...
    if ((game.__ivanIntroRemaining ?? 0) <= 0 && game.__ivanGuardGap >= IVAN_MAX_GAP) {
...
proto.step = function (dt: number) {
  ...
  if (wasPlaying && game.mode === 'playing') {
    const actualHit = game.__actualHitItem;
