...
const makeZeus = (): ZeusState => {
  ...
  const lightningMat = new T.MeshBasicMaterial({ color: '#fff7a8', transparent: true, opacity: .98 });
  ...
  // Zeus stands on the cloud: simple classical marble figure with beard and laurel/crown.
  ...
  for (let i=0;i<7;i++) {
    const leaf = new T.Mesh(new T.SphereGeometry(.07,8,6), gold);
    const a = -1.15 + i*.38; leaf.position.set(Math.sin(a)*.4,.3+Math.cos(a)*.08,Math.cos(a)*.08); leaf.scale.set(.7,1.5,.5); head.add(leaf);
  }

  const arm = (x:number, z:number, rot:number) => {
    const upper = new T.Mesh(new T.CapsuleGeometry(.11,.58,4,10), marble); upper.position.set(x,1.22,z); upper.rotation.z=rot; upper.castShadow=true; body.add(upper);
    return upper;
  };
  arm(-.62,.02,-.42);
  const raised = arm(.62,.05,.72);
  const hand = new T.Mesh(new T.SphereGeometry(.15,12,8), marble); hand.position.set(.88,1.7,.05); hand.castShadow=true; body.add(hand);
  raised.rotation.z=.72;

  const bolt = new T.Group(); bolt.name='ZeusLightning'; bolt.visible=false;
  const makeBolt = (finalStrike:boolean) => {
    bolt.clear();
    const x0 = group.position.x + .88, y0 = group.position.y + 2.05, z0 = group.position.z;
    const runner = (group.userData.runner as T.Group | undefined);
    const tx = runner ? runner.position.x : 0, ty = runner ? runner.position.y + 1.0 : 1.0, tz = runner ? runner.position.z : 1.2;
    const points:T.Vector3[]=[]; const n=finalStrike?7:6;
    for(let i=0;i<=n;i++) { const t=i/n; points.push(new T.Vector3(T.MathUtils.lerp(x0,tx,t)+(i===0||i===n?0:(Math.random()-.5)*.55),T.MathUtils.lerp(y0,ty,t),T.MathUtils.lerp(z0,tz,t)+(i===0||i===n?0:(Math.random()-.5)*.22))); }
    const line = new T.Line(new T.BufferGeometry().setFromPoints(points), lightningMat); line.computeLineDistances(); line.scale.setScalar(finalStrike?1.35:1); bolt.add(line);
  };
  (bolt as any).__makeBolt = makeBolt;
  group.add(bolt);
...
const ensureZeus = (game:any):ZeusState => {
  ...
  const state = makeZeus();
  game.__zeus = state;
  game.scene.add(state.group);
  state.group.userData.runner = game.runner;
  state.group.visible = false;
  return state;
};

const showZeus = (game:any, intro=false) => {
  const z = ensureZeus(game);
  z.active = true;
  z.intro = intro ? 3.6 : 0;
  z.grace = intro ? 0 : 60;
  z.strikeAt = intro ? 999 : .25;
  z.strikeFlash = 0;
  z.group.visible = true;
  z.group.position.set(game.runner.position.x, 4.0, intro ? -3.5 : 5.0);
  z.group.rotation.set(0,0,0);
};

const hideZeus = (game:any) => {
  const z = ensureZeus(game);
  z.active=false; z.intro=0; z.grace=0; z.strikeFlash=0; z.bolt.visible=false; z.group.visible=false;
};

const lightning = (game:any, fatal=false) => {
  const z = ensureZeus(game);
  z.group.userData.runner = game.runner;
  z.bolt.visible=true; z.strikeFlash=fatal?.28:.18;
  (z.bolt as any).__makeBolt(fatal);
  game.shake=Math.max(game.shake||0,fatal?.22:.045);
  game.tone(fatal?95:180,fatal?.38:.16,fatal?28:60,'triangle');
...
proto.start = function() {
  ...
  const z=ensureZeus(game);
  showZeus(game,true);
  game.onToast('Зевс наблюдает за тобой');
  return undefined;
};
...
proto.step = function(dt:number) {
  ...
  // A first recoverable hit is converted back into gameplay. A second hit while Zeus is active is fatal.
  if(wasPlaying && !activeBefore) game.onOver=(()=>{}) as any;
  try { previousStep.call(this,dt); }
  finally { game.onOver=originalOnOver; }

  if(wasPlaying && game.mode==='over' && recoverableHit(game,checkedBefore)) {
    if(activeBefore) {
      lightning(game,true);
      z.active=false; z.grace=0;
      game.onToast('Карающий Зевс поразил тебя');
      if(game.onOver) game.onOver({...game.stats});
      return;
    }
    game.mode='playing'; game.shake=Math.max(game.shake||0,.08);
    showZeus(game,false);
    game.onToast('Удар выдержан — Зевс карает');
    return;
  }

  if(game.mode==='playing') {
    if(z.intro>0) {
      z.intro=Math.max(0,z.intro-dt);
      z.group.position.x=T.MathUtils.damp(z.group.position.x,game.runner.position.x,7,dt);
      z.group.position.z=T.MathUtils.damp(z.group.position.z,-2.0,5,dt);
      if(z.intro<=0) hideZeus(game);
    } else if(z.active && z.grace>0) {
      z.grace=Math.max(0,z.grace-dt);
      const elapsed=60-z.grace;
      const behind=Math.min(7.0,5.0+elapsed*.055);
      z.group.position.x=T.MathUtils.damp(z.group.position.x,game.runner.position.x,8,dt);
      z.group.position.z=T.MathUtils.damp(z.group.position.z,behind,1.7,dt);
      z.group.position.y=4.0+Math.sin(game.phase*.8)*.08;
      z.strikeAt-=dt;
      if(z.strikeAt<=0) { lightning(game,false); z.strikeAt=.85+Math.random()*.75; }
